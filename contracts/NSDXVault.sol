...
contract NSDXVault is Ownable, Pausable {
    ...
    IERC20 public immutable token; // NSDX token
    ...
    /**
     * @notice Constructor
     * @param _token: NSDX token contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), MAX_UINT256);
    }
    ...
    /**
     * @notice Deposits funds into the NSDX Vault
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the NSDX Vault
     */
     ...
     */
    function calculateHarvestNSDXRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingNSDX(0, address(this));
     ...
     */
    function calculateTotalPendingNSDXRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingNSDX(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the NSDX Vault
